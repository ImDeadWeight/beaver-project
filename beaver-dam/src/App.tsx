...
// process exclusively through the beaverAPI bridge defined in the preload
...
const getAPI = (): BeaverAPI | undefined => (window as unknown as { beaverAPI?: BeaverAPI }).beaverAPI
const api = (): BeaverAPI => {
  const a = getAPI()
  if (!a) throw new Error('beaverAPI not available — preload may have failed')
  return a
}
...
const DEFAULT_CONFIG: LlamaConfig = {
  ...
  gpuLayers: 0, port: 8080, host: '0.0.0.0', networkMode: true,
...
export default function App() {
  ...
  useEffect(() => {
    const a = getAPI()
    if (!a) {
      setStatusMsg('ERROR: beaverAPI not found — preload script may have failed to load.')
      return
    }
  ...
  // Beaver Log app (because the app registers the beaver:// URI scheme in its
  ...
  // would just open the browser instead of the Beaver Log app.
  ...
  async function scanHardware() {
    ...
    setConfig(prev => ({
      ...
      gpuLayers: specs.gpu.cudaAvailable ? 24 : 0,
    }))
  }
    ...
    <div className="flex flex-col h-screen bg-zinc-950 text-white font-mono text-sm overflow-hidden">
      ...
      <header className="flex items-center justify-between px-5 py-3 bg-zinc-900 border-b border-zinc-800 shrink-0">
        <h1 className="text-lg font-bold tracking-wide">
          Beaver <span className="text-orange-500">/ LlamaCpp Launcher</span>
        </h1>
          ...
          <section>
            ...
            {networkMode && (
              <div className="mt-3 space-y-2">
                ...
                {qrDataUrl && (
                  <div>
                    <p className="text-xs text-zinc-500 mb-1">Scan with Android camera to open Beaver Log and connect automatically:</p>
        ...
        <main className="flex-1 flex flex-col overflow-y-auto p-5 gap-5">
          ...
          <section className="bg-zinc-900 rounded-lg p-4 border border-zinc-800">
            ...
            <div className="mt-4 grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-zinc-500 mb-1">N-CPU-MoE <span className="text-zinc-600">(MoE models only)</span></label>
                <input
                  type="number"
                  ...
                  placeholder="Leave blank for non-MoE"
