...
public class NetworkDiscoveryPlugin extends Plugin {
            ...
            for (int host = 1; host <= 254; host++) {
                ...
                tasks.add(() -> {
                    if (isPortOpen(ip, BEACON_PORT, connectTimeout)) {
                        return probeBeaverDamBeacon(ip);
                    }
                    return null;
                });
            }
    ...
    /**
     * Probes the Beaver Dam identity beacon at http://ip:8765.
     * Returns a server JSObject with url/ip/port if the host is a running Beaver Dam
     * instance, or null if it is not or the beacon response is invalid.
     */
    private JSObject probeBeaverDamBeacon(String ip) {
        try {
