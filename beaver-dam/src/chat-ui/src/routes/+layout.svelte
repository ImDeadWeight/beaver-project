<script lang="ts">
	...
	import BeaverLoadingScreen from '$lib/components/app/BeaverLoadingScreen.svelte';
	...
	// when the user scans a QR code from Beaver Dam. I handle it in the layout
	...
	// to open Beaver Log when the camera sees one of these codes.
		...
		try {
			...
			if (parsed.protocol === 'beaver:' && parsed.hostname === 'connect') {
				...
				if (serverUrl) {
					...
					toast.success('Connected to Beaver Dam!', { duration: 3000 });
				}
			}
		} catch { /* invalid URL */ }
	}
	...
	async function initApp() {
		...
		if (needsScan) {
			...
			try {
				// Both Android (Capacitor) and Beaver Log Windows expose the same
				...
				const discovery: NetworkDiscoveryPlugin = isCapacitorAndroid()
					? NetworkDiscovery
					: (window as unknown as { beaverLogAPI: { network: NetworkDiscoveryPlugin } })
		...
		// On native platforms (Beaver Log Windows/Android), only fetch if we actually
...
<Tooltip.Provider delayDuration={TOOLTIP_DELAY_DURATION}>
	...
	{#if !appReady}
		...
		>
			<BeaverLoadingScreen phase={loadingPhase} />
		</div>
	{/if}
